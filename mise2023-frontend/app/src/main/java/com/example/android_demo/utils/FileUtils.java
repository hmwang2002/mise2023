...
public class FileUtils {
        ...
        if (scheme == null)
            ...
            if (cursor != null) {
                if (cursor.moveToFirst()) {
                    int columnIndex = cursor.getColumnIndexOrThrow(Media.DATA);
    ...
    public static Uri getImageContentUri(Context context, File imageFile) {
        ...
        Cursor cursor = context.getContentResolver().query(Media.EXTERNAL_CONTENT_URI,
                new String[] { Media._ID }, Media.DATA + "=? ",
                new String[] { filePath }, null);
        if (cursor != null && cursor.moveToFirst()) {
            int id = cursor.getInt(cursor.getColumnIndex(MediaStore.MediaColumns._ID));
            Uri baseUri = Uri.parse("content://com.google.android.apps.photos.content");
            cursor.close();
